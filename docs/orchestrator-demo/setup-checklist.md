...
- [ ] **Запустить сервис** — выполни `npm run dev`, bridge поднимется на `http://127.0.0.1:7071`.
- [ ] **Проверить здоровье** — выполни `Invoke-RestMethod http://127.0.0.1:7071/health` и убедись, что в ответе есть `ok: true` и `service: "cli-bridge"`.
